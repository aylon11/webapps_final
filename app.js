...
redis_client.on('connect', function(err){
    ...
    redis_client.hmset('admin',['password','admin'], function(err){
        if(err){
...
app.post('/user/add/:name/:pwd', function(req,res,next){
    ...
    redis_client.hgetall(user_name, function(err,obj){
        if (obj){
            res.send('User name already exists')
        }
        else{
            redis_client.hmset(user_name,['password',pwd], function(err,reply){
                if (err){
                    console.log(err)
                }
                console.log(reply);
...
app.get('/user/get/:name', function(req, res, next){
    ...
    redis_client.hgetall(name, function(err, obj){
        // console.log(obj)
        res.send(obj)
  });
})
